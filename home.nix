...
in {
  ...
  programs.zsh = {
    ...
    initContent = lib.mkMerge [
      ...
      ''
        ...
        # Bun completions (runtime install, kept out of Nix).
        [ -s "$HOME/.bun/_bun" ] && source "$HOME/.bun/_bun"
        if [ -d "$HOME/.bun/bin" ]; then
          export PATH="$HOME/.bun/bin:$PATH"
        fi

        # Doom Emacs launcher, if present.
        if [ -d "$HOME/dotfiles/.config/emacs/bin" ]; then
          export PATH="$HOME/dotfiles/.config/emacs/bin:$PATH"
        fi
  ...
  # Ghostty itself is installed via Homebrew cask (nixpkgs' ghostty is Linux-only).
  # We still let home-manager own the config file.
  programs.ghostty = {
    enable = true;
    package = null;
    enableZshIntegration = true;
  ...
  # Raw config links.
  # .p10k.zsh is generated by `p10k configure`; immutable link is fine.
  home.file.".p10k.zsh".source = ./.p10k.zsh;

  # Mutable-symlink dirs: home-manager's file walker chokes on
  # mkOutOfStoreSymlink-to-directory, so we make the links ourselves during
  # activation. These stay editable in place (LazyVim writes lazy-lock.json,
  # htop rewrites htoprc).
  #
  # Karabiner-Elements is NOT linked here — it atomically replaces
  # karabiner.json on every UI change (write-temp-then-rename), which
  # breaks symlinks. The config lives authoritatively at
  # ~/.config/karabiner/karabiner.json; copy it into this repo when it
  # changes (see README).
    ...
    lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      linkMutable() {
        ...
        if [ -e "$target" ] && [ ! -L "$target" ]; then
          echo "home-manager: warning: $target is not a symlink; move it aside and re-run."
          return 0
        fi
        run --silence ln -sfn "$src" "$target"
      }
      run --silence mkdir -p "$HOME/.config"
      linkMutable "${dotfilesDir}/.config/nvim" "$HOME/.config/nvim"
      linkMutable "${dotfilesDir}/.config/htop" "$HOME/.config/htop"
    '';
}
