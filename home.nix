...
in {
  ...
  # LazyVim writes lazy-lock.json back to its config dir, so this must be writable.
  xdg.configFile."nvim".source = mkLink "${dotfilesDir}/.config/nvim";

  # Karabiner writes backups into this directory, so keep it writable.
  xdg.configFile."karabiner".source = mkLink "${dotfilesDir}/.config/karabiner";

  # htop rewrites its own config file when settings change.
  xdg.configFile."htop/htoprc".source = mkLink "${dotfilesDir}/.config/htop/htoprc";
}
