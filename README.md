...
Tools that write to their own config dir (`~/.config/nvim`, `~/.config/karabiner`, `~/.config/htop`) are linked as mutable symlinks back to this repo. If existing dirs are in the way:

```sh
for d in ~/.config/nvim ~/.config/karabiner ~/.config/htop; do
...
- **Karabiner-Elements**: `karabiner.json` is authoritative; edit via the UI or directly, changes land in the repo
